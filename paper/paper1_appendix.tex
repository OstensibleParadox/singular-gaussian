...
\subsection{Asymptotic Balancing Diagnostic}
\label{app:experiments:ablation}

We validate the asymptotic balancing normalization of Proposition~\ref{prop:exact_calibration} in its decoupled reference channel. With incoming target-coordinate blocks and off-diagonal drift coupling set to zero, the target covariance is exactly $\Sigma_{\lambda,22} = d_\lambda / (2\lambda)$. Four candidate target-diffusion families are tested:
\begin{enumerate}
